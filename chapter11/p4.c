 ...
 * Modify poker.c program of Section 10.5 by passing pointers
 * to function analyze_hand so that analyze_hand deposit
 * the boolean values of the hand value in the belonging pointers
 * then main will give hand value.
 *
 */
...
void analyze_hand (bool *rf, bool *s, bool *al, bool *fl, bool *f, bool *t, int *p);
...
int main (void) {
  for (;;) {
    read_cards();
    analyze_hand(&royal_flush, &straight, &ace_low, &flush, &four, &three, &pairs);
    print_result();
  }
  return 0;
}
...
void analyze_hand (bool *rf, bool *s, bool *al, bool *fl, bool *f, bool *t, int *p) {
  ...
  *p=0;
  ...
  for (rank=0; rank<NUM_RANKS; rank++) {
    if (rank_times[rank]==4) *f=true;
    if (rank_times[rank]==3) *t=true;
    if (rank_times[rank]==2) (*p)++;
  }
}
