 ...
 * using the 12-hour clock format). The program then displays the departure and arrival times
...
int main (void) {
  ...
  /* Compute total minutes since midnight based on clock format */
  if ((c1 == 'a') || (c1 == 'A')) {
    im = hours * 60 + minutes;
  } else if ((c1 == 'p') || (c1 == 'P')) {
      im = (12+hours) * 60 + minutes;
  } else {
